"""Redis Streams Consumer - Simple & Correct Version"""
...
class RedisStreamConsumer:
    """Redis Streams를 사용하여 기사 처리 작업을 소비하는 컨슈머"""
    ...
    def _process_messages(self, messages: List) -> List[Dict[str, Any]]:
        ...
        for stream_name, stream_messages in messages:
            for record_id, data in stream_messages:
                try:
                    # articleId에서 불필요한 따옴표 제거
                    if 'articleId' in data:
                        data['articleId'] = data['articleId'].strip('"')
    ...
    def get_pending_info(self) -> Dict[str, Any]:
        """
        Consumer Group의 Pending 메시지 요약 정보를 조회합니다.
        ...
        try:
            ...
            logger.error(f"Pending 정보 조회 중 오류: {str(e)}")
            return {}
    ...
    def get_pending_messages(self, count: int = 10) -> List[Dict[str, Any]]:
        """
        Pending 상태의 메시지를 조회합니다.
        ...
        try:
            ...
            logger.info(f"Pending 메시지 {len(result)}개 조회")
            return result
            
        except Exception as e:
            logger.error(f"Pending 메시지 조회 중 오류: {str(e)}")
            return []

    def claim_message(self, record_id: str, min_idle_time: int = 60000):
        """
        Pending 메시지를 현재 컨슈머로 재할당합니다.
        
        Args:
            record_id: 메시지 ID
            min_idle_time: 최소 유휴 시간 (밀리초)
        """
        try:
            if not self.redis_client:
                raise RuntimeError("Redis가 연결되지 않았습니다")
            ...
            claimed = self.redis_client.xclaim(
                ...
                consumername=self.consumer_name,
                min_idle_time=min_idle_time,
                message_ids=[record_id]
            )
            
            if claimed:
                logger.info(f"메시지 재할당 완료: {record_id}")
                return claimed[0]
            return None
            
        except Exception as e:
            logger.error(f"메시지 재할당 실패 (recordId={record_id}): {str(e)}")
            return None
    ...
    def trim_stream(self, max_len: int = 10000, approximate: bool = True):
        ...
        try:
            ...
            logger.info(f"Stream trimming 완료 (max_len={max_len}, approximate={approximate})")
            
        except Exception as e:
            logger.error(f"Stream trimming 중 오류: {str(e)}")
