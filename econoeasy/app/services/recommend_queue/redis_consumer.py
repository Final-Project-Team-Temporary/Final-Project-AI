"""Redis Streams Consumer - YouTube Recommend용"""

import redis
import logging
from typing import Optional, Dict, Any, List
from ...core.config import settings
from ...models.schemas import KeywordRecommendStreamMessage

logger = logging.getLogger(__name__)


class RecommendRedisConsumer:
    """Redis Streams를 사용하여 YouTube 추천 작업을 소비하는 컨슈머"""

    def __init__(self):
        self.redis_client: Optional[redis.Redis] = None
        self.stream_key = settings.REDIS_RECOMMEND_STREAM_KEY
        self.consumer_group = settings.REDIS_RECOMMEND_CONSUMER_GROUP
        self.consumer_name = settings.REDIS_RECOMMEND_CONSUMER_NAME
    ...
    def connect(self):
        ...
        try:
            ...
            logger.info("Redis 연결 성공 (Recommend Consumer)")

            # Consumer Group 생성 또는 확인
            self._ensure_consumer_group()

        except Exception as e:
            logger.error(f"Redis 연결 실패 (Recommend Consumer): {str(e)}")
            raise
    ...
    def disconnect(self):
        ...
        if self.redis_client:
            ...
            logger.info("Redis 연결 종료 (Recommend Consumer)")
    ...
    def read_messages(self, count: int = 10, block: int = 5000) -> List[Dict[str, Any]]:
        ...
        Returns:
            메시지 리스트 [{"id": record_id, "data": KeywordRecommendStreamMessage}, ...]
        """
        try:
        ...
        for stream_name, stream_messages in messages:
            for record_id, data in stream_messages:
                try:
                    ...
                    # Pydantic 모델로 변환
                    message = KeywordRecommendStreamMessage(**data)
                    result.append({
                        "id": record_id,
                        "data": message
                    })
...
# 전역 Recommend Redis Consumer 인스턴스
recommend_redis_consumer = RecommendRedisConsumer()
